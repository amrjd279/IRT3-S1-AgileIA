...
## Étape 8 : Journal IA, première entrée
Objectif : prendre l'habitude qui sera évaluée pendant tout le semestre.

1. Ouvrez le template dans journaux_ia/ et écrivez votre première entrée, en vous appuyant sur un des appels que vous venez de faire.

* Le journal IA a été créé avec une première entrée réelle : ``journal_ia.md``
...
L’entrée documente :
* L’appel à POST ``/chat/stream``
* Le résultat obtenu ;
* La correction de l’erreur ``422 JSON decode error`` ;
* ce qui a été accepté et corrigé.

Vérifications effectuées :
* une réponse de Gemini a bien été obtenue ;
* ``.env`` est bien ignoré par Git ;
* aucune clé API n’est inscrite dans le journal.